...
Названия пиши этот текст:⚠Берешь промпт — обязательно ставь лайк ❤ на пост и оставляй любой комментарий! Описания делай максимально полезными и точными 
для последующего создания промпта для генерации изображения. Если на фото есть текст уточней на каком языке изображен текст.
...
Перед отправкой вставляем нужный промт в коментариях
ВАЖНО ПРОЧИТАТЬ
❗ Промт всегда можно и нужно менять под себя:
цвет волос, глаз, одежду, позу, настроение и т.д.
⚠ МОЁ ЕДИНСТВЕННОЕ И ОБЯЗАТЕЛЬНОЕ УСЛОВИЕ:
Берешь промпт — обязательно ставь лайк ❤ на пост и оставляй любой комментарий! 
